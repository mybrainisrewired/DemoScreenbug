...
public final class MovieLayoutAnimation {
    private static void appearAnim(LayoutTransition transition) {
        Animator anim = ObjectAnimator.ofFloat(null, "rotationY", new float[]{90.0f, 0.0f}).setDuration(transition.getDuration(ClassWriter.COMPUTE_FRAMES));
        anim.addListener(new AnimatorListenerAdapter() {
            public void onAnimationEnd(Animator anim) {
                ((View) ((ObjectAnimator) anim).getTarget()).setRotationY(0.0f);
            }
        });
        transition.setAnimator(ClassWriter.COMPUTE_FRAMES, anim);
    }
    ...
    private static void disappearAnim(LayoutTransition transition) {
        Animator anim = ObjectAnimator.ofFloat(null, "rotationY", new float[]{0.0f, 90.0f}).setDuration(transition.getDuration(JsonWriteContext.STATUS_OK_AFTER_SPACE));
        anim.addListener(new AnimatorListenerAdapter() {
            public void onAnimationEnd(Animator anim) {
                ((View) ((ObjectAnimator) anim).getTarget()).setRotationX(0.0f);
            }
        });
        transition.setAnimator(JsonWriteContext.STATUS_OK_AFTER_SPACE, anim);
    }
