...
public abstract class LocalMediaItem extends MediaItem {
    ...
    public MediaDetails getDetails() {
        ...
        details.addDetail(JsonWriteContext.STATUS_OK_AFTER_SPACE, DateFormat.getDateTimeInstance().format(new Date(this.dateTakenInMs)));
        if (Utils.isValidLocation(this.latitude, this.longitude)) {
            details.addDetail(JsonWriteContext.STATUS_EXPECT_VALUE, new Object{this.latitude, this.longitude});
        }
        if (this.fileSize > 0) {
            details.addDetail(Type.OBJECT, Long.valueOf(this.fileSize));
        }
        return details;
    }
