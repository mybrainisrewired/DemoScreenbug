...
public class WMTFileInstall {
    ...
    private static final String SCRIPTCCMD_OTA_SYSTEM = "scriptcmd_wmt_ota_install_system.wmt!sh";
    ...
    static void wmtInstallOta(Context context, String type) {
        if (type == null) {
            Log.v(TAG, "install fail !");
        } else if (type != null && type.equals(FileInstall.USER_TYPE)) {
            ScriptCmd4wmt8850.ExecWmtScriptCmd(FileManager.copyAssetsScript(context, scriptSavePath, SCRIPTCCMD_APP_USER));
        } else if (type != null && type.equals(FileInstall.SYSTEM_TYPE)) {
            ScriptCmd4wmt8850.ExecWmtScriptCmd(FileManager.copyAssetsScript(context, scriptSavePath, SCRIPTCCMD_OTA_SYSTEM));
        }
    }
