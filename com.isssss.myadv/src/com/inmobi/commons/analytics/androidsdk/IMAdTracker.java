...
public final class IMAdTracker {
    ...
    private boolean a(String str) {
        boolean z = false;
        try {
            String preferences = FileOperations.getPreferences(InternalSDKUtil.getContext(), AdTrackerConstants.IMPREF_FILE, RequestBuilderUtils.KEY_MK_SITE_ID);
            if (InternalSDKUtil.getContext() == null || preferences == null || preferences.trim().equals(Preconditions.EMPTY_ARGUMENTS)) {
                Log.debug(AdTrackerConstants.IAT_LOGGING_TAG, AdTrackerConstants.MSG_INITIALIZATION_INCOMPLETE);
                return z;
            } else if (str == null || Preconditions.EMPTY_ARGUMENTS.equals(str.trim())) {
