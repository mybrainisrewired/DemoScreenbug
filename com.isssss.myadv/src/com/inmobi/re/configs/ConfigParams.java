...
public class ConfigParams {
    ...
    public void setFromMap(Map<String, Object> map) {
        try {
            ...
            String replace = h.replace("\\", Preconditions.EMPTY_ARGUMENTS);
            replace = replace.substring(1, replace.length() - 1);
            if (replace.contains(",")) {
                String[] split = replace.split(",");
                int i = 0;
                while (i < split.length) {
                    if (split[i].substring(1, split[i].length() - 1).equals(Preconditions.EMPTY_ARGUMENTS)) {
                        throw new IllegalArgumentException();
                    }
                    i++;
                }
            } else {
                if (new String[]{replace}[0].equals(Preconditions.EMPTY_ARGUMENTS)) {
                    throw new IllegalArgumentException();
                }
            }
