    ...
    public synchronized IMAIClickEventList getClickEvents(int i) {
        ...
        try {
            ...
            do {
                ...
                if (1 == nRows.getInt(MMAdView.TRANSITION_UP)) {
                    clickData.setPingWv(true);
                } else {
                    clickData.setPingWv(false);
                }
                if (1 == nRows.getInt(MMAdView.TRANSITION_DOWN)) {
                    clickData.setFollowRedirect(true);
                } else {
                    clickData.setFollowRedirect(false);
                }
                clickData.setRetryCount(nRows.getInt(MMAdView.TRANSITION_RANDOM));
                clickData.setTimestamp(nRows.getLong(ApiEventType.API_MRAID_GET_ORIENTATION_PROPERTIES));
