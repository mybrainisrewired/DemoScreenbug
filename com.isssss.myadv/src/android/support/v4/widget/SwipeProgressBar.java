...
final class SwipeProgressBar {
    ...
    private void drawCircle(Canvas canvas, float cx, float cy, int color, float pct) {
        ...
        canvas.drawCircle(BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, cx, this.mPaint);
        canvas.restore();
    }
    ...
    void draw(Canvas canvas) {
        ...
        if (this.mRunning || this.mFinishTime > 0) {
            ...
            if (this.mRunning || now - this.mFinishTime < 1000) {
                float clearRadius = ((float) (width / 2)) * INTERPOLATOR.getInterpolation((((float) ((now - this.mFinishTime) % 1000)) / 10.0f) / 100.0f);
                this.mClipRect.set(((float) cx) - clearRadius, BitmapDescriptorFactory.HUE_RED, ((float) cx) + clearRadius, (float) height);
