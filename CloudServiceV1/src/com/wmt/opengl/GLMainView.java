...
public class GLMainView extends GLSurfaceView implements Renderer, GLPrivContext {
    ...
    private void onDrawFrameLocked(GL10 gl10) {
        ...
        if (this.mFrameNumber >= 5) {
            int delay = this.mIsEmulator ? 50000 : KeyTouchInputEvent.ABS_MT_WIDTH_MAJOR;
            if (this.mTextureManager.checkLoadedQueue(realdt < ((long) delay) ? FLAG_NEED_LAYOUT : ItemAnimation.CUR_ALPHA)) {
                requestRender();
            }
    ...
    protected void init(Context context) {
        ...
        this.mRootView.addView(this.mToast, GLView.ZORDER_TOAST);
        setBackgroundDrawable(null);
        setEGLConfigChooser(FragmentManagerImpl.ANIM_STYLE_FADE_ENTER, FragmentManagerImpl.ANIM_STYLE_FADE_EXIT, 5, 0, MmInfo.MMINFO_MEM_SIZE, 0);
